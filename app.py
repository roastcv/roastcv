...
if not has_report and _nav not in ("keywords", "templates", "roasts", "about", "contact"):
    ...
    if run_clicked:
        if not resume_file and not jd_text.strip():
            ...
            st.error(f"❌ File too large. Maximum size is {MAX_RESUME_SIZE_MB} MB — please compress your file.")
            ...
            try:
                ...
                _progress_heading = (
                    "### ⚙️ Building Resume from LinkedIn & Analyzing..."
                    if is_linkedin_mode
                    else "### ⚙️ Stage 1: Quick Analysis in Progress..."
                )

                with analysis_placeholder.container():
                    st.markdown("---")
                    st.markdown(_progress_heading)
                    st.caption("⚡ ATS Score & Resume Health will appear in ~20 seconds. Cover Letter & Rewrite load after.")

                    progress_bar   = st.progress(0)
                    step_container = st.empty()
                    tip_container  = st.empty()

                    # Stage 1 progress steps (3 total)
                    STAGE1_STEPS = [
                        "Parsing Resume",
                        "Resume Reader + JD Analyzer",
                        "ATS · Audit · Gap · HR · Recruiter · Hiring Manager · Interview Coach",
                    ]

                    def update_stage1_progress(step, total, label):
                        progress_bar.progress(min(step / total, 1.0) * 0.6)  # max 60% for stage1
                        steps_html = ""
                        for i, s in enumerate(STAGE1_STEPS):
                            if i < step:
                                steps_html += (
                                    f'<div class="step-indicator">'
                                    f'<div class="step-dot-done"></div> '
                                    f'<span style="color:#15803d;text-decoration:line-through;opacity:0.6">{s}</span>'
                                    f'</div>'
                                )
                            elif i == step:
                                steps_html += (
                                    f'<div class="step-indicator">'
                                    f'<div class="step-dot-active"></div> '
                                    f'<span style="color:#2563eb;font-weight:600">{s}</span>'
                                    f'</div>'
                                )
                            else:
                                steps_html += (
                                    ...
                                    f'<span style="color:#94a3b8">{s}</span>'
                                    f'</div>'
                                )
                    ...
                    # ── STAGE 1: Fast agents ──────────────────────────────
                    stage1_report = run_pipeline_stage1(
                        tmp_path,
                        jd_text=jd_text,
                        progress_callback=update_stage1_progress,
                    )

                    # Stage 1 done — show partial results immediately
                    progress_bar.progress(0.6)
                    tip_container.empty()
                    step_container.markdown(
                        '<div style="color:#22c55e;font-weight:600;padding:0.5rem 0;">'
                        '✅ ATS Score + Resume Health ready! Loading Rewrite & Cover Letter...</div>',
                        unsafe_allow_html=True,
                    )

                    st.session_state.stage1_report = stage1_report
                    st.session_state.report = stage1_report   # show partial results NOW
                    st.session_state.report_company_name = company_name
                    st.session_state.stage2_running = True
                    st.session_state.nav_section = "overview"

                    # ── STAGE 2: Slow chain (Rewrite → Humanizer → Cover Letter) ──
                    STAGE2_STEPS = [
                        "Resume Rewrite",
                        "Humanizer",
                        "Cover Letter",
                    ...
                    def update_stage2_progress(step, total, label):
                        # 60% to 100% bar for stage 2
                        progress_bar.progress(0.6 + min(step / total, 1.0) * 0.4)
                        steps_html = ""
                        for i, s in enumerate(STAGE2_STEPS):
                            if i < step:
                                steps_html += (
                                    f'<div class="step-indicator">'
                                    f'<div class="step-dot-done"></div> '
                                ...
                                steps_html += (
                                    f'<div class="step-indicator">'
                                    f'<div class="step-dot-active"></div> '
                                    f'<span style="color:#2563eb;font-weight:600">{s} (loading...)</span>'
                                    f'</div>'
                                )
                            else:
                                steps_html += (
                    ...
                    complete_report = run_pipeline_stage2(
                        stage1_report,
                        company_name=company_name,
                        progress_callback=update_stage2_progress,
                    )
                    ...
                    if is_linkedin_mode:
                        st.success("✅ LinkedIn profile converted and full analysis complete!")
                    else:
                        st.success("✅ Full analysis complete! Rewrite & Cover Letter ready.")

                st.session_state.report = complete_report
                st.session_state.stage2_complete = True
                st.session_state.stage2_running = False
                st.session_state.running = False
                st.session_state.nav_section = "overview"

                # ── Wall of Roasts: ask permission ──────────────────────
